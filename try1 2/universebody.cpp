...
void UniverseBody::renderLabel(QPainter& painter) const
{
    ...
    //draw the label
    painter.setPen(m_color);
    QString detail = getName().c_str();
    painter.drawText(QRectF(x, y, 150.0, 50.0), detail);
}
...
void UniverseBody::accept(Visitor* visitor,int event)
{
    visitor->visit(this,event);
}

void UniverseBody::accept(Visitor *visitor, double x, double y){
    return visitor->visit(this,x,y);
}
